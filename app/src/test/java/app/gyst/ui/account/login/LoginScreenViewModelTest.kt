...
class LoginScreenViewModelTest {
    ...
    ): LoginScreenViewModel = LoginScreenViewModel(loginUseCase, biometrics, appPreferenceRepository, userAccountRepository)
    ...
    fun creates_state_from_results() {
        ...
        val userAccountRepository: UserAccountRepository = mockk {
            every { userWithProfile } returns mockk {
                every { profileId } returns null andThen null andThen UUID.randomUUID()
            }
        }
        val viewModel = createViewModel(
        ...
        assertThat(actualStates).isEqualTo(
            listOf(
                ...
                LoginScreenState.Navigate(R.id.nav_introduction_screen.asNavDirection()),
                LoginScreenState.Navigate(R.id.nav_financial_overview.asNavDirection()),
